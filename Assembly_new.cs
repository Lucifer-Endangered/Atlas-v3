...
namespace InventorAssemblyExporter
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            string baseOutputRoot = @"C:\Users\hrish\Downloads\Assemblies for Training ML model\Output";
        ...
        static void ProcessAssembly(string iamPath, string outputRoot)
        {
            ...
            string outputPath = System.IO.Path.Combine(
                outputRoot, 
                System.IO.Path.GetFileNameWithoutExtension(iamPath) + ".json");
            ...
            ComponentOccurrences occurrences, List<ComponentNode> nodes, string parentPath)
        {
            foreach (ComponentOccurrence occ in occurrences)
            {
                ...
                {
                    try { ExtractOccurrences((ComponentOccurrences)occ.SubOccurrences, nodes, nodeId); } catch { }
                }
            }
        }
        ...
        static ConstraintEdge ExtractConstraint(AssemblyConstraint constraint)
        {
            ...
            try
            {
                if (constraint.OccurrenceOne != null)
                    edge.node_one_id = constraint.OccurrenceOne.Name;
            }
            catch { }

            try
            {
                if (constraint.OccurrenceTwo != null)
                    edge.node_two_id = constraint.OccurrenceTwo.Name;
            }
            catch { }
        ...
        static JointEdge ExtractJoint(AssemblyJoint joint)
        {
            ...
            try
            {
                dynamic def = joint.Definition;

                try
                {
                    jEdge.linear_start_cm = def.LinearPositionStartLimit.Value;
                    jEdge.linear_end_cm = def.LinearPositionEndLimit.Value;
                    jEdge.has_linear_limit = true;
                }
                catch
                {
                    jEdge.has_linear_limit = false;
                }

                try
                {
                    jEdge.angular_start_rad = def.AngularPositionStartLimit.Value;
                    jEdge.angular_end_rad = def.AngularPositionEndLimit.Value;
                    jEdge.has_angular_limit = true;
                }
                catch
                {
                    jEdge.has_angular_limit = false;
                }
            }
            catch { }
